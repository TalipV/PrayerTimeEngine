    ...
    ) : DbContext(options)
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        // These entities carry IDs assigned by the external API, so EF must not treat the
        // int primary key as an auto-increment column (replaces [DatabaseGenerated(None)],
        // which used to live on the domain entities themselves).
        modelBuilder.Entity<FaziletCountry>().Property(x => x.ID).ValueGeneratedNever();
        modelBuilder.Entity<FaziletCity>().Property(x => x.ID).ValueGeneratedNever();
